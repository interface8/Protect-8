...
export default function KnowledgeCard({ article, onClick }: KnowledgeCardProps) {
  ...
  return (
    <Card
      ...
      onClick={onClick}
    >
      {/* Category Pill - Light Gray */}
      <div className="inline-block px-2.5 py-0.5 rounded-full text-xs font-medium text-[#554116] bg-[#e8e8e8] mb-2">
        {categoryLabel}
      </div>

      {/* Title */}
      <h3 className="font-semibold text-[#554116] text-sm md:text-base mb-1.5 line-clamp-2 flex-1">
        {article.title}
      </h3>

      {/* Excerpt */}
      <p className="text-xs md:text-sm text-[#0a0a0a]/60 line-clamp-2 mb-2.5">
        {article.excerpt}
      </p>

      {/* Divider Line */}
      <div className="border-t border-[#554116]/10 mb-2"></div>
