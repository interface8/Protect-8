...
export default function HomeAvailableNow() {
  ...
  return (
    <div className="w-full bg-[#f5f3f0]">
      <div className="w-[90%] max-w-[944px] mx-auto pt-6 md:pt-9 pb-6 md:pb-8">
        <div className="flex items-center justify-between mb-3 md:mb-4">
          ...
          <Link
            href="/find-a-lawyer"
            className="text-sm text-[#c4922a] hover:underline font-medium flex items-center gap-0.5"
          >
        ...
        <div className="space-y-2 md:space-y-3">
          {lawyers.slice(0, 3).map((lawyer) => (
            <Card
              key={lawyer.id}
              className="p-4 gap-0 shadow-none rounded-xl border-black/[0.06] hover:border-[#c4922a]/40 transition-colors cursor-pointer bg-white"
              onClick={() => router.push(`/find-a-lawyer/${lawyer.id}`)}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 md:gap-4">
                  <div className="relative w-10 h-10 md:w-12 md:h-12 flex-shrink-0">
                    <div className="w-full h-full rounded-xl bg-[#c4922a]/20 flex items-center justify-center overflow-hidden">
                      {lawyer.avatar ? (
                        <img
                          src={lawyer.avatar}
                          alt={lawyer.name}
                  ...
                  <div>
                    <h4 className="font-medium text-[#0a0a0a] text-sm md:text-base">
                      {lawyer.name}
                    </h4>
                ...
                <div className="flex flex-col items-end gap-0.5 md:gap-1">
                  {lawyer.rating > 0 && (
                    <div className="flex items-center gap-1">
                      <Star className="w-3.5 h-3.5 fill-[#c4922a] text-[#c4922a]" />
