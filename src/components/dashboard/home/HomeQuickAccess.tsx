...
export default function HomeQuickAccess() {
  return (
    <div className="w-full bg-[#f5f3f0]">
      <div className="w-[90%] max-w-[944px] mx-auto pt-4 md:pt-8">
        <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-4 text-left">
          Quick Access
        </h3>
        <div className="grid grid-cols-3 gap-2 md:gap-3">
          {quickAccessItems.map((item) => {
            ...
            return (
              <Link key={item.title} href={item.href}>
                <Card className="p-2 md:p-5 gap-0 shadow-none rounded-xl border-black/[0.06] bg-white hover:border-[#c4922a]/40 transition-colors cursor-pointer">
                  <div className="flex flex-col items-start gap-1 md:gap-2.5">
                    <div className="w-10 h-10 md:w-9 md:h-9 bg-[#0a0a0a] rounded-full md:rounded-xl flex items-center justify-center flex-shrink-0">
                      <Icon className="w-4 h-4 text-white" />
                    </div>
                    <div className="text-left">
                      {/* Mobile: line breaks, Desktop: straight text */}
                      <h4 className="font-medium text-[#0a0a0a] text-sm md:text-[15px] leading-snug whitespace-pre-line md:whitespace-normal">
                        {displayTitle}
                      </h4>
                      <p className="text-xs text-gray-500 hidden sm:block md:mt-0.5">
                        {item.description}
                      </p>
                    </div>
                  </div>
